...
#ifdef TEST2
		//y=-5x^2+5000x-1249500
		double y = -5 * pow(X1, 2) + 5000 * X1 - 1249500;
		return y;
#endif
	}
...
#ifdef TEST2
		//y=-5x^2+5000x-1249500
		double y = -5 * pow(p1.m_X, 2) + 5000 * p1.m_X - 1249500;
		return y;
#endif
	}
		...
		for (; i < max_iter; ++i) {
			...
			if (flag) {
				...
				if (besiege_flag) {
					...
					//update();
#ifdef TEST1
					// if(sqrt(pow(m_HeadWolfIter->m_Position.m_X - m_TargetPos.m_X, 2) + pow(m_HeadWolfIter->m_Position.m_Y - m_TargetPos.m_Y, 2))<1.0){
...
#endif
#ifdef TEST2
		cout << "最优结果为：" << "x=" << m_HeadWolfIter->getX() << ",y=" << calcFitness(m_HeadWolfIter->m_Position, m_TargetPos) << endl;
#endif
		cv::waitKey(0);
	}
